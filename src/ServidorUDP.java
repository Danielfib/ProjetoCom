	...
	public void run() {
		...
		try {
			...
			while (true) {
				...
				if (p.syn) {
					Pacote synAck = new Pacote(0, 0, serverIsn, p.numSeq + 1, false, false, true, false, 0, 0, "Teste 2".getBytes());
...
//							} else {
							...
							serverSocket.send(new DatagramPacket(ackDuprikred, ackDuprikred.length, ipDestinoInet, portaDestino));
